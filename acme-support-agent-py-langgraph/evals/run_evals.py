"""Native-SDK eval runner for the LangGraph variant.

Scores each case with the built-in criteria and attaches scores to the trace via
score(). This is the baseline; the -deepeval and -ragas variants swap the judging
layer for an external library while keeping the same dataset and golden paths.
...
def main() -> None:
    ...
    for r in results:
        ...
        print(f"      tools={r['tools']}  correct={r['correctness']:.0f}  "
              f"traj={r['trajectory']:.0f}  {r['latency_s']}s")
    print("=" * 72 + "\n")
